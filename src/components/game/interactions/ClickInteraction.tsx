...
export const ClickInteraction = ({ prompt, onComplete }: ClickInteractionProps) => {
  // 'clicking' → 'carousel' → done (onComplete)
  const [phase, setPhase] = useState<'clicking' | 'carousel'>('clicking');
  ...
  const handleClick = () => {
    ...
    if (newCount >= requiredClicks) {
      // pequeno delay para a animação de "completou" aparecer antes de trocar
      setTimeout(() => setPhase('carousel'), 600);
    }
  };
      ...
      <div className="flex flex-col items-center gap-6">
        ...
        <AnimatePresence mode="wait">
          {clickCount < requiredClicks ? (
            ...
            // brief "completed" flash antes do carrossel aparecer
            <motion.div
              key="done"
  ...
  return (
    <AnimatePresence>
      ...
      >
        <PhotoCarousel onFinish={onComplete} />
      </motion.div>
    </AnimatePresence>
  );
};